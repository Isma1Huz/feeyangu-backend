            ...
            <SidebarGroupContent>
              <SidebarMenu>
                {section.items.map((item) => {
                  ...
                  const linkContent = (
                    <NavLink to={item.url} end={!item.url.includes('health') && !item.url.includes('portfolio') && !item.url.includes('pt-meetings')}
                      className={cn('flex items-center gap-3 rounded-md px-3 py-2 text-sm transition-colors', 'text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground')}
                      activeClassName="bg-sidebar-accent text-sidebar-accent-foreground font-semibold border-l-[3px] border-primary">
