...
import { MOCK_HEALTH_PROFILES, MOCK_STUDENTS, MOCK_HEALTH_UPDATE_REQUESTS } from '@/lib/mock-data';
...
const SchoolHealth: React.FC = () => {
  ...
  const activeConditions = MOCK_HEALTH_PROFILES.reduce((s, p) => s + p.conditions.filter(c => c.status === 'active').length, 0);
  const allergies = MOCK_HEALTH_PROFILES.reduce((s, p) => s + p.allergies.length, 0);
  const vaccDue = MOCK_HEALTH_PROFILES.reduce((s, p) => s + p.vaccinations.filter(v => v.status !== 'up_to_date').length, 0);
  const incidents = MOCK_HEALTH_PROFILES.reduce((s, p) => s + p.incidents.length, 0);
  ...
  // Critical alerts
  const criticalAlerts = MOCK_HEALTH_PROFILES.flatMap(p => {
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" className="gap-2" onClick={() => navigate('/school/health/settings')}><Settings className="h-4 w-4" />Settings</Button>
          <Button variant="outline" className="gap-2" onClick={() => navigate('/school/health/trip-sheet')}><FileText className="h-4 w-4" />Trip Sheet</Button>
          <Button className="gap-2" onClick={() => navigate('/school/health/records')}><Users className="h-4 w-4" />All Records</Button>
        </div>
      </div>
      ...
      {MOCK_HEALTH_UPDATE_REQUESTS.length > 0 && (
        <Card className="shadow-sm border-warning/30">
          ...
          <CardContent>
            {MOCK_HEALTH_UPDATE_REQUESTS.map(req => {
