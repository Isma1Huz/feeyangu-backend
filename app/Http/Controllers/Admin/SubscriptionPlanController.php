...
class SubscriptionPlanController extends Controller
{
    ...
    /**
     * Duplicate an existing plan (clones settings and module assignments).
     */
    public function duplicate(int $id): RedirectResponse
    {
        ...
        DB::transaction(function () use ($plan) {
            ...
            foreach ($plan->modules as $module) {
                $syncData[$module->id] = ['is_included' => $module->pivot->is_included];
            }
    ...
    public function previewChanges(Request $request, int $id): \Illuminate\Http\JsonResponse
    {
        ...
        $added   = array_diff($newModuleIds, $currentModuleIds);
        $removed = array_diff($currentModuleIds, $newModuleIds);

        $affectedSchoolCount = $plan->schools()->count();

        return response()->json([
            'affected_schools' => $affectedSchoolCount,
            'modules_added'    => Module::whereIn('id', $added)->pluck('name'),
            'modules_removed'  => Module::whereIn('id', $removed)->pluck('name'),
        ]);
    }
}
